...
Это значит, что для одинаковык улючей не может быть разных данных.
...
| **Опционалы** | Почему `словари` всегда возвращают опциональные значения |
...
Перед нами поставили задачу – сгруппировать всех персон по имени – т.е. все Пети, должны быть собраны в одном месте, ровка как и все Василии и так далее. В таком случае нам идеально подойдёт Словарь. 
...
/// Давайте пройдёмся по всем персонам
    for person in persons {
        ...
        if var array = usersDictionary[person.name] {
            /// Добавим текущую персону к существующим
            array.append(person)
...
4. **Со звёздочкой** Посчитайте средний бал для каждого из студентов в словаре. 
