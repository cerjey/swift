...
<details>
<summary><b>🔰 Вопрос 1: Какой индекс у ПОСЛЕДНЕГО элемента массива из 5 элементов?</b></summary>

<br>

> [!TIP]
> **✅ Ответ: 4**  
> *Пояснение:* последний элемент всегда имеет индекс `array.count - 1`.  
> Если в массиве 5 элементов, то индексы: 0, 1, 2, 3, 4.

<br>
</details>

<details>
<summary><b>💥 Вопрос 2: Что произойдет при выполнении кода?</b></summary>
...
> *Пояснение:* обращение по несуществующему индексу вызывает **fatal error**.  
> Индекс 3 — это четвертый элемент, но в массиве всего 3 элемента (индексы 0, 1, 2).

<br>
</details>

<details>
<summary><b>🧮 Вопрос 3: Что выведет этот код?</b></summary>
...
> **🔢 Ответ: 3**  
> *Пояснение:*  
> 1. Было 5 элементов: [1, 2, 3, 4, 5]  
> 2. `remove(at: 1)` удаляет элемент с индексом 1 (число 2) → осталось [1, 3, 4, 5]  
> 3. `removeFirst()` удаляет первый элемент (число 1) → осталось [3, 4, 5]  
> 4. `count` = 3
...
<details>
<summary><b>🔄 Вопрос 4: Какая разница между `sort()` и `sorted()`?</b></summary>

<br>

| Метод | Действие | Пример |
|-------|----------|--------|
| `sort()` | Меняет **оригинальный** массив | `array.sort()` → array изменяется |
| `sorted()` | Создает **новый** отсортированный массив | `let new = array.sorted()` → array не меняется |

> [!IMPORTANT]
> **✅ Ответ: `sort()` меняет массив, `sorted()` создает новый**

<br>
</details>

<details>
<summary><b>🔎 Вопрос 5: Что вернет `[1, 2, 3, 2, 1].firstIndex(of: 2)`?</b></summary>

<br>

```swift
let array = [1, 2, 3, 2, 1]
let index = array.firstIndex(of: 2)
print(index) // ?
```

> [!TIP]
> **✅ Ответ: 1**  
> *Пояснение:* `firstIndex(of:)` возвращает индекс **ПЕРВОГО** вхождения элемента.  
> Число 2 встречается дважды (индексы 1 и 3), но метод возвращает только первый — **1**.

<br>
</details>

<details>
<summary><b>🛡️ Вопрос 6: Как безопасно получить последний элемент массива?</b></summary>

<br>

| ❌ Опасно | ✅ Безопасно |
|-----------|--------------|
| `array[array.count - 1]` (упадет, если массив пуст) | `array.last` (возвращает `nil` для пустого массива) |

> [!NOTE]
> **✅ Ответ: `array.last`**  
> *Пояснение:* свойство `last` возвращает **Optional**, поэтому безопасно даже для пустого массива:
> ```swift
...
<details>
<summary><b>🎭 Вопрос 7: Можно ли хранить в одном массиве `Student` и `Teacher`?</b></summary>

<br>

> [!TIP]
> **✅ Ответ: Можно, если обернуть в протокол**  
> *Пояснение:* создаем протокол `Person`, который реализуют обе структуры,  
> и объявляем массив типа `[Person]`:
>
> ```swift
> protocol Person { var name: String { get } }
> struct Student: Person { let name: String; let scores: [Int] }
> struct Teacher: Person { let name: String; let subject: String }
>
> let people: [Person] = [Student(name: "Анна", scores: [5,4]), Teacher(name: "Иван", subject: "Math")]
> ```
...
<details>
<summary><b>🧪 Вопрос 8: Что выведет этот код?</b></summary>

<br>

```swift
let array = [1, 2, 3, 4, 5]
let result = array.filter { number in
    number % 2 == 0
}
print(result)
```
...
> [!NOTE]
> **✅ Ответ: [2, 4]**  
> *Пояснение:* `filter` оставляет только элементы, удовлетворяющие условию:  
> * 1 % 2 = 1 → не подходит  
> * 2 % 2 = 0 → ✅ оставляем  
> * 3 % 2 = 1 → не подходит  
> * 4 % 2 = 0 → ✅ оставляем  
...
<details>
<summary><b>📭 Вопрос 9: Что произойдет при выполнении?</b></summary>
...
> [!TIP]
> **✅ Ответ: nil**  
> *Пояснение:* `first` для пустого массива возвращает `nil`.  
> Это безопасно — программа не упадет, просто выведет `nil`.

<br>
</details>

<details>
<summary><b>⬅️ Вопрос 10: Как добавить элемент в начало массива?</b></summary>
...
// ❌ Так добавляется в КОНЕЦ
array.append(10)     // [1, 2, 3, 4, 5, 10]

// ✅ Так добавляется в НАЧАЛО
array.insert(10, at: 0)  // [10, 1, 2, 3, 4, 5]
```

> [!IMPORTANT]
> **✅ Ответ: `array.insert(10, at: 0)`**  
> *Пояснение:* `insert(at: 0)` вставляет элемент в начало массива (индекс 0),  
> сдвигая все существующие элементы вправо.
...
| 🏆 Результат | 🎯 Количество правильных ответов |
|--------------|----------------------------------|
| 🌟 Отлично | 10/10 |
| 👍 Хорошо | 8-9/10 |
| 📚 Нужно повторить | 5-7/10 |
| 🔄 Начните сначала | 0-4/10 |

> 💡 **Совет:** Если на какой-то вопрос ответили неверно — перечитайте соответствующий раздел документации!
