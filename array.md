...
> ⚠️ **Запомните:** 
> Последний элемент всегда имеет индекс `array.count - 1`
...
- [ ] Да
- [x] Нет, массивы должны быть одного типа
- [ ] Можно через запятую
...
0. Создай тип данных `Phone`
1. Создай массив, который хранит в себе два объекта с типом данных `Phone`
2. Создай тип данных `Notebook`
3. Попробуй добавить в массив с телефонами объект с типом данных `Notebook`
4. Создай протокол, который позволит хранить в массиве и `Phone` и `Notebook`
5. Добавь в конец массива `Notebook`
6. Найди индекс `Phone`, который тебе нужен (например модель `iPhone 13`)
7. Удали из массива элемент под индексом, который нашла на предыдущем шаге
8. Поменяй поле из протокола у любого `Notebook` (Которое есть в протоколе)
9. Поменяй специфичное только для `Phone` поле (Которого нет в протоколе)
10. Радуйся

Решение можете присылать [сюда](t.me/swift_learning)
